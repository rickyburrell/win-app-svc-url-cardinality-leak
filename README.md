...
> [!WARNING]
> **Update — 9 June 2026:** While we wait for the IIS team to fix the underlying `nativerd.dll` leak, it can be mitigated by disabling failed request tracing — `iisfreb.dll` is at fault. In [`deploy/resources.bicep`](deploy/resources.bicep), set `failedRequestsTracing: { enabled: false }`.
